...
projectModalEl.addEventListener('show.bs.modal', event => {
    ...
    document.getElementById("projectname-input3").value = "";
    document.getElementById("projectname-input4").value = "";
        ...
        document.getElementById("task-list").innerHTML +=
            ...
            <td>\
                <div class="d-flex align-items-center">\
                    ...
                    <img width="auto" height="80" src="' + singleTodo.thumbnail + '" />\
                </div>\
            </td>\
            <td style="max-width: 180px;">' + singleTodo.newsUrl + '</td>\
            <td>' + singleTodo.newsTitle + '</td>\
            <td>' + singleTodo.newsPublishDate + '</td>\
            <td>' + singleTodo.newsText + '</td>\
            <td id="mediaNo" class="displayNone">' + singleTodo.no + '</td>\
            <td>\
            <div class="hstack gap-2">\
                <button class="btn btn-sm btn-soft-info edit-list" data-bs-toggle="modal" data-bs-target="#createProjectModal" data-edit-id='+ singleTodo.no + ' data-edit-newsPublishDate=' + singleTodo.newsPublishDate + ' data-edit-text=' + idx + '><i class="ri-pencil-fill align-bottom"></i></button>\
                <button class="btn btn-sm btn-soft-danger remove-list" data-bs-toggle="modal" data-bs-target="#removeTaskItemModal" data-remove-id='+ singleTodo.no + ' data-remove-name=' + singleTodo.newsUrl + '><i class="ri-delete-bin-5-fill align-bottom"></i></button>\
            </div>\
            </td>\
        </tr>';
...
function editTodoList() {
    ...
    Array.from(document.querySelectorAll(".edit-list")).forEach(function (elem) {
        elem.addEventListener('click', function (event) {
            ...
            $('#projectname-input3').removeClass("is-invalid");
            $('#projectname-input3-feedback').fadeOut(0);
            $('#projectname-input4-feedback').fadeOut(0);

            getEditid = elem.getAttribute('data-edit-id');
            getEditName = manyTodosP[elem.getAttribute('data-edit-text')].newsUrl;
            getEditTitle = manyTodosP[elem.getAttribute('data-edit-text')].newsTitle;
            getEditNewsText = manyTodosP[elem.getAttribute('data-edit-text')].newsText;
            getEditNewsPublishDate = elem.getAttribute('data-edit-newsPublishDate'); 
            getEditNewsImage = manyTodosP[elem.getAttribute('data-edit-text')].thumbnail;

            $("#projectname-input").val(getEditName);
            $("#projectname-input2").val(getEditNewsText);
            $("#projectname-input3").val(getEditNewsPublishDate);
            $("#projectname-input4").val(getEditTitle);
            ...
            document.getElementById("editProject").addEventListener("click", function () {
                ...
                imgData.append("oriTitle", getEditName);
                imgData.append("url", $.trim($('#projectname-input').val()));
                imgData.append("text", $.trim($('#projectname-input2').val()));
                imgData.append("publish", $.trim($('#projectname-input3').val()));
                imgData.append("title", $.trim($('#projectname-input4').val()));

                $.ajax({
                    url: '/DashBoard/EditNews',
                    type: "POST",
                    processData: false,
                    contentType: false,
                    data: imgData,
...
document.querySelector("#addNewProject").addEventListener("click", function () {
    ...
    if ($('#partnerImg')[0].files[0] == null) {
        $('#projectname-input4-feedback').fadeIn(0);
        return;
    }
    else {
        $('#projectname-input4-feedback').fadeOut(0);
    ...
    if ($.trim($('#projectname-input3').val()) == "") {
        $('#projectname-input3').addClass("is-invalid");
    ...
    if ($.trim($('#projectname-input').val()) != "" && $.trim($('#projectname-input2').val()) != "")
    {
        ...
        imgData.append("formFile", $('#partnerImg')[0].files[0]);
        imgData.append("url", $.trim($('#projectname-input').val()));
        imgData.append("newsText", $.trim($('#projectname-input2').val()));
        imgData.append("newsPublish", $.trim($('#projectname-input3').val()));
        imgData.append("title", $.trim($('#projectname-input4').val()));


        $.ajax({
            url: '/DashBoard/SetNews',
            type: "POST",
            processData: false,
            contentType: false,
            data: imgData,
