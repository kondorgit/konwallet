...
projectModalEl.addEventListener('show.bs.modal', event => {
    ...
    document.getElementById("projectname-input0").value = "";
    document.getElementById("projectname-input00").value = "";
...
var drake = dragula([document.getElementById("task-list")], {
    ...
    if (mediaNo.length > 0) {
        $.post("/DashBoard/SetAdvisorOrderby", {
            noArr: mediaNoArr
        },
        function (data) {
        ...
        document.getElementById("task-list").innerHTML +=
            ...
            <td>\
                <div class="d-flex align-items-center">\
                    ...
                    <img width="auto" height="80" src="' + singleTodo.img + '" />\
                </div>\
            </td>\
            <td>' + singleTodo.name + '</td>\
            <td>' + singleTodo.nameEn + '</td>\
            <td>' + singleTodo.position + '</td>\
            <td>' + singleTodo.text.replace(/\n/gi, "<br>") + '</td>\
            <td>' + singleTodo.textEn.replace(/\n/gi, "<br>") + '</td>\
            <td id="mediaNo" class="displayNone">' + singleTodo.no + '</td>\
            <td>\
            <div class="hstack gap-2">\
                <button class="btn btn-sm btn-soft-info edit-list" data-bs-toggle="modal" data-bs-target="#createProjectModal" data-edit-id='+ singleTodo.no + ' data-edit-title=' + singleTodo.name + ' data-edit-text=' + idx + ' data-edit-image=' + singleTodo.img + '><i class="ri-pencil-fill align-bottom"></i></button>\
                <button class="btn btn-sm btn-soft-danger remove-list" data-bs-toggle="modal" data-bs-target="#removeTaskItemModal" data-remove-id='+ singleTodo.no + ' data-remove-title=' + singleTodo.name + '><i class="ri-delete-bin-5-fill align-bottom"></i></button>\
            </div>\
            </td>\
        </tr>';
...
function editTodoList() {
    ...
    Array.from(document.querySelectorAll(".edit-list")).forEach(function (elem) {
        elem.addEventListener('click', function (event) {
            ...
            getEditTitle2 = manyTodosP[elem.getAttribute('data-edit-text')].nameEn;
            position = manyTodosP[elem.getAttribute('data-edit-text')].position;
            getEditNewsText = manyTodosP[elem.getAttribute('data-edit-text')].text;
            getEditNewsTextEn = manyTodosP[elem.getAttribute('data-edit-text')].textEn;
            getEditNewsImage = elem.getAttribute('data-edit-image');

            $("#projectname-input").val(getEditTitle);
            $("#projectname-input0").val(getEditTitle2);
            $("#projectname-input00").val(position);
            $("#projectname-input2").val(getEditNewsText);
            $("#projectname-input4").val(getEditNewsTextEn);
            ...
            document.getElementById("editProject").addEventListener("click", function () {
                ...
                if ($.trim($('#projectname-input').val()) != "" && $.trim($('#projectname-input2').val()) != "") {
                    ...
                    imgData.append("title2", $.trim($('#projectname-input0').val()));
                    imgData.append("position", $.trim($('#projectname-input00').val()));
                    imgData.append("text", $.trim($('#projectname-input2').val()));
                    imgData.append("textEn", $.trim($('#projectname-input4').val()));
                    imgData.append("id", getEditid);


                    $.ajax({
                        url: '/DashBoard/EditAdvisor',
                        type: "POST",
                        processData: false,
                        contentType: false,
                        data: imgData,
